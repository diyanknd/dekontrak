<div :class="[$store.app.menu=='horizontal' ? 'max-w-[1704px] mx-auto xxl:px-0 xxl:pt-8':'',$store.app.stretch?'xxxl:max-w-[92%] mx-auto':'']" class="p-3 md:p-4 xxl:p-6 space-y-4 xxl:space-y-6" x-data="{  TotalInvoice: 'All' }">
  <!-- Breadcrumb -->
  ...
  <div class="grid grid-cols-12 gap-4 xxl:gap-6">
    ...
    <div class="col-span-12 grid grid-cols-12 4xl:grid-cols-10 gap-4 xxl:gap-6" x-data="{jenis_nilai : 'pagu', activeTab:'pagu'}">
            ...
            <div class="grow">
             <div class="bb-dashed-n30">
              <div class="col-span-12 flex max-w-max flex-wrap gap-4 rounded-xl md:rounded-[40px] border border-neutral-30 p-3 dark:border-neutral-500">
                ...
                <button @click="jenis_nilai ='pagu', activeTab='pagu',TotalInvoice='pagu',totalpagu='<?php echo $nilai_pagu ?>'" :class="activeTab=='pagu' ?'bg-primary-300 text-neutral-0': 'bg-neutral-20 dark:bg-neutral-903'" class="rounded-full capitalize px-4 py-2 text-sm font-semibold lg:px-6 lg:py-2.5">Nilai Pagu</button>

                <button @click="jenis_nilai ='kontrak',activeTab='kontrak',TotalInvoice='kontrak'" :class="activeTab=='kontrak' ?'bg-primary-300 text-neutral-0': 'bg-neutral-20 dark:bg-neutral-903'" class="rounded-full capitalize px-4 py-2 text-sm font-semibold lg:px-6 lg:py-2.5">Nilai Kontrak</button>
              </div>
            </div>
          </div>
    ...
    <div class="col-span-12 sm:col-span-6 lg:col-span-4 xxxl:col-span-3 4xl:col-span-2">
      <div class="white-box xl:p-6">
        ...
        <div class="flex justify-between items-center">
          <div class="grow">
            <h4 class="mb-4" x-text="activeTab=='pagu' ? '<?php echo number_format($get_pagu_fisik) ?>' : '<?php echo number_format($get_kontrak_fisik) ?>'"></h4>
            <p class="m-text" x-text="activeTab=='pagu' ? '<?php echo $get_pagu_fisik_count." Paket" ?>' : '<?php echo $get_pagu_fisik_count." Paket" ?>'"></p>
          </div>
  ...
  <div class="col-span-12">
   <div x-data="{
    ...
    <div class="bb-dashed-n30">
      <div class="col-span-12 flex max-w-max flex-wrap gap-4 rounded-xl md:rounded-[40px] border border-neutral-30 p-3 dark:border-neutral-500">
        <button @click="TotalInvoice ='All',activeTab='all', filterInvoiceByStatus('all')" :class="activeTab=='all' ?'bg-primary-300 text-neutral-0': 'bg-neutral-20 dark:bg-neutral-903'" class="rounded-full capitalize px-4 py-2 text-sm font-semibold lg:px-6 lg:py-2.5">all</button>

        <button @click="TotalInvoice ='tender', activeTab='tender', filterInvoiceByStatus('tender')" :class="activeTab=='tender' ?'bg-primary-300 text-neutral-0': 'bg-neutral-20 dark:bg-neutral-903'" class="rounded-full capitalize px-4 py-2 text-sm font-semibold lg:px-6 lg:py-2.5">Tender</button>

        <button @click="TotalInvoice ='non_tender',activeTab='non_tender', filterInvoiceByStatus('non_tender')" :class="activeTab=='non_tender' ?'bg-primary-300 text-neutral-0': 'bg-neutral-20 dark:bg-neutral-903'" class="rounded-full capitalize px-4 py-2 text-sm font-semibold lg:px-6 lg:py-2.5">Non Tender</button>
      </div>
    </div>
        ...
        <button>
          <div x-data="dropdown" class="relative flex justify-center" @click.away="close()">
            ...
            <ul class="horiz-option" :class="isOpen ? 'show' : 'hide'">
              <li>
                <span class="flex items-center hover:text-primary-300 gap-2 cursor-pointer rounded px-3 py-1.5 text-sm leading-normal duration-300 hover:bg-primary-50">
    ...
    <!-- search results -->
    <div x-show="activeTab != 'all' || searchQuery.length > 0" class="flex flex-wrap justify-between items-center gap-3 mb-4 xl:mb-6">
  ...
  <tbody>
    ...
    :class="invoice.checked ? '!bg-primary-300/10' : 'bg-neutral-0 dark:bg-neutral-904'">
        ...
        <td class="px-6" :class="dense? 'py-2': 'py-2 lg:py-3'">
          <span x-show="invoice.status=='tender'" class="py-2 px-5 rounded-3xl text-xs text-success-300 bg-success-300/10 border border-neutral-30 dark:border-neutral-500">Tender</span>
          <span x-show="invoice.status=='non_tender'" class="py-2 px-5 rounded-3xl text-xs text-error-300 bg-error-300/10 border border-neutral-30 dark:border-neutral-500">Non Tender</span>
        </td>
        <td>
          <div x-data="dropdown" class="relative flex justify-center" @click.away="close()">
            <i class="las la-ellipsis-v cursor-pointer text-2xl" @click="toggle()"></i>
            <ul class="horiz-option" :class="isOpen?'show':'hide'">
              <li>
                <a href="invoice-details.html" class="flex items-center hover:text-primary-300 gap-2 cursor-pointer rounded px-3 py-1.5 text-sm leading-normal duration-300 hover:bg-primary-50">
      ...
      <div class="mt-6 flex items-center gap-5 justify-center flex-col md:flex-row md:justify-between whitespace-nowrap">
        ...
        <div class="flex flex-col sm:flex-row justify-center sm:justify-between gap-5">
          ...
          <div class="f-center gap-4">
            <p x-text="`${(currentPage - 1) * pageSize + 1}-${Math.min(currentPage * pageSize, filteredInvoices.length)} of ${filteredInvoices.length}`"></p>
